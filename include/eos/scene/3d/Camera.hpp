...
class Camera : public WorldObject{
    public:
        ...
        void apply_view_matrix(const std::initializer_list<std::shared_ptr<eos::Shader>>& shaders, std::string_view name = "view") const;

        void apply_projection_matrix(const std::initializer_list<std::shared_ptr<eos::Shader>>& shaders, std::string_view name = "projection", float zNear = 0.1f, float zFar = 100.0f) const;
    };
}

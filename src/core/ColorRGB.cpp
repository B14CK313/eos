...
eos::ColorRGB::ColorRGB(float red, float green, float blue, float alpha) : red_(red), green_(green), blue_(blue),
                                                                           alpha_(alpha) {}

eos::ColorRGB::ColorRGB(unsigned short red, unsigned short green, unsigned short blue, unsigned short alpha) : red_(
		red / 255.0f), green_(green / 255.0f), blue_(blue / 255.0f), alpha_(alpha / 255.0f) {}
...
eos::ColorRGB operator ""_rgba(unsigned long long int hex) {
	eos::ColorRGB colorRgb{};
	colorRgb.red_ = ((hex >>= 8u) & 0xFFu) / 255.0f;
	colorRgb.green_ = ((hex >>= 8u) & 0xFFu) / 255.0f;
	colorRgb.blue_ = ((hex >> 8u) & 0xFFu) / 255.0f;
	colorRgb.alpha_ = ((hex) & 0xFFu) / 255.0f;
	return colorRgb;
}

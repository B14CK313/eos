...
eos::Font::Font(const FT_Library& freetypeLib, const std::string& path, float size) : path_(path) {
	if (FT_Error err = FT_New_Face(freetypeLib, path.c_str(), 0, &fontFace_)) {
...
void eos::Font::cache(char32_t codepoint) {
	if (auto it = characterMap_.find(codepoint); it != characterMap_.end()) {
		...
		characterMap_.emplace(codepoint, Character{textureAtlas_.insert(bitmap.buffer, {bitmap.width, bitmap.rows}),
		                                           {glyph->bitmap_left, glyph->bitmap_top}, glyph->advance.x});
	}
}

...
eos::Text::Text(std::experimental::observer_ptr<Graphics> graphics, std::shared_ptr<Font> font) : Text(graphics, font,
                                                                                                       std::make_shared<Shader>(
		                                                                                                       "res/text.vert",
		                                                                                                       "res/text.frag")) {}

eos::Text::Text(std::experimental::observer_ptr<Graphics> graphics, std::shared_ptr<Font> font,
                std::shared_ptr<Shader> shader) : graphics_{graphics}, font_(font), shader_(shader) {
	shader_->use();
...
std::u32string eos::Text::setup_render(const std::string& text) const {
	shader_->use();
	glm::vec2 windowSize = graphics_->size();
